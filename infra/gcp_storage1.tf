...
resource "google_compute_instance_group_manager" "gcp_storage1_mig" {
  name        = "gcp-storage1-mig"
...
resource "google_compute_autoscaler" "gcp_storage1_autoscaler" {
  ...
  zone   = "us-central1-a"
  target = google_compute_instance_group_manager.gcp_storage1_mig.self_link
